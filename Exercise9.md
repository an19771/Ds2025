...
# this relation is positive, however it is not very strong
```
...
## Your code here
print(ss.pearsonr(df_pokemon['Attack'], df_pokemon["Total"]))
print(ss.spearmanr(df_pokemon['#'], df_pokemon['Generation']))
print(ss.kendalltau(df_pokemon['Total'], df_pokemon['Sp. Atk']))
```

    PearsonRResult(statistic=np.float64(0.7362106519949294), pvalue=np.float64(1.8465097387937472e-137))
    SignificanceResult(statistic=np.float64(0.9835812364964281), pvalue=np.float64(0.0))
...
# Qualitative vs Quantitative: Box plot or Violin plot
# Qualitative vs Qualitative: Grouped bar chart or heatmap
# Ranked data: Strip plot or line plot with ordered categories
```
...
sns.scatterplot(x=df_sales['No_of_Customers'], y=df_sales['Sales'])
#sns.barplot(x=df_sales['Product_Quality'], y=df_sales['Sales'])
print(ss.pearsonr(df_sales['No_of_Customers'], df_sales['Sales']))

ct = pd.crosstab(df_sales['Product_Quality'], df_sales['Store_Type'])
chi2, p, dof, expected = chi2_contingency(ct)
print(chi2, p, dof, expected)

```

    1.8958333333333333 0.38754757378490456 2 [[2.28571429 1.71428571]
