...
```python
df_pokemon = pd.read_csv("https://raw.githubusercontent.com/KeithGalli/pandas/master/pokemon_data.csv")
df_pokemon.head(10)
```
...
```python
### Your code here
#A histogram cannot be made for 'Type 1' because it contains categorical data 
# rather than numerical values, which is what histograms require.
```
...
```python
### Your code here
#The distribution looks mostly normal with most values centered around 10, but it has outliers at value 21 that are clearly separated from the main group of data.
```
